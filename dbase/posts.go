...
func GetAllPosts(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var p models.Posts
		err2 := rows.Scan(&p.ID, &p.AuthorID, &p.Title, &p.Content)
		if err2 != nil {
...
func AddNewPost(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	ReceiveJSON(r, &post)
	st, err1 := db.Prepare(`INSERT INTO Posts (AuthorID,Title,Content) VALUES (?,?,?)`)
	if err1 != nil {
		fmt.Println("AddNewPost db.Prepare", err1)
		http.Error(w, err1.Error(), http.StatusInternalServerError)
		return
	}
	_, err2 := st.Exec(post.AuthorID, post.Title, post.Content)
	if err2 != nil {
...
func GetPostByID(db *sql.DB, w http.ResponseWriter, r *http.Request, postID int) {
	...
	err := rows.Scan(&post.ID, &post.AuthorID, &post.Title, &post.Content)
	if err != nil {
...
func EditPostByID(db *sql.DB, w http.ResponseWriter, r *http.Request, postID int) {
	...
	ReceiveJSON(r, &new)
	st, err2 := db.Prepare(`UPDATE Posts SET AuthorID = ?, Title = ?, Content = ? where ID = ?`)
	if err2 != nil {
		fmt.Println("EditPostByID db.Prepare:", err2)
		http.Error(w, err2.Error(), http.StatusInternalServerError)
		return
	}
	_, err3 := st.Exec(new.AuthorID, new.Title, new.Content, postID)
	if err3 != nil {
